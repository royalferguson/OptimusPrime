...
import pandas as pd 
import numpy ad np 
import matplotlib.backends.backend_tkagg import FigureCanvasTkAgg 
...
class VisualizerDisplay(tk.Tk):
	def __init__(self, *args, **kwargs):
		...
		for F in (StartPage, PageOne, PageTwo, PageThree, PageFour, PageFive, PageSix, PageSeven, PageEight, PageNine):
			...
			self.frames[F] = frameframe.grid(row=0, column=0, sticky="nsew")
...
class StartPage(tk.Frame):
	...
	button = tkk.Button(self, text="Score vs Iteration", width=30, command=lambda:controller.show_frame(PageOne))

	button.pack()
		button = tkk.Button(self, text="Scores(log) vs Iteration", width=30, command=lambda:controller.show_frame(PageTwo))
	button.pack()
		button = tkk.Button(self, text="DV Values", width=30, command=lambda:controller.show_frame(PageThree))
	button.pack()
		button = tkk.Button(self, text="DV-Ave Values", width=30, command=lambda:controller.show_frame(PageFour))
	button.pack()
		button = tkk.Button(self, text="DV-RMS Values", width=30, command=lambda:controller.show_frame(PageFive))
	button.pack()
		button = tkk.Button(self, text="DV Values - Heat Map", width=30, command=lambda:controller.show_frame(PageSix)
	button.pack()
		button = tkk.Button(self, text="DV-AVE Values - Heat Map", width=30, command=lambda:controller.show_frame(PageSeven))
	button.pack()
		button = tkk.Button(self, text="sqrt (DV-RMS) Values - Heat Map", width=30, command=lambda:controller.show_frame(PageEight))
	button.pack()
		button = tkk.Button(self, text="DV-RMS Values -Heat Map", width=30, command=lambda:controller.show_frame(PageNine))
	button.pack()
